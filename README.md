...
- Interactive alignment map on the Alignment Calculator and Find Alignments modes: observer and target markers, the observer→target bearing line, a dashed Sun/Moon azimuth direction line, a tolerance sector, an alignment status overlay, and automatic fit-to-alignment with a "Fit alignment" button
...
Enter a target latitude/longitude, date, and time. The target can be picked by typing a landmark name (searched against OpenStreetMap Nominatim) or by clicking/dragging a marker on the map; landmark coordinates and manual edits stay in sync. The app detects the timezone from the coordinates, computes the Sun/Moon position at that moment, and reports the azimuth, altitude, and how closely a line drawn from the target toward the object aligns with the target-to-north reference. The interactive map visualises the observer, target, observer→target line, object azimuth direction line, and alignment status. Inputs changed after calculating do not move the map until you recalculate — the map always shows the last calculated alignment.

### Find Alignments
Pick a Sun/Moon object, a target bearing, and a date range. The app searches for times when the object's azimuth matches the target bearing within the selected tolerance, refines each hit to peak alignment, and rates quality with an alignment-star score. Filters like "full moon ±1 day" are available for the Moon. Results are selectable cards; the first result is auto-selected and shown on the interactive map, and selecting a different result updates the map and refits it to the chosen alignment.
...
- The alignment map is read-only — target selection happens in the target picker map or by manual coordinates.
